// Advisor piece and its moving rule.
//
...
namespace HoxMobile
{
    public class AdvisorPiece : Piece
    {
        public AdvisorPiece(PieceColor c, Position p) : base(PieceType.Advisor, c, p) { }

        public override Piece Clone()
        {
            return new AdvisorPiece(m_color, m_position.Clone());
        }
        ...
        public override bool CanMoveTo(Position newPos)
        {
            ...
            if (!(Math.Abs(newPos.X - m_position.X) == 1 && Math.Abs(newPos.Y - m_position.Y) == 1))
                return false;
        ...
        public override List<Position> GetPotentialNextPositions()
        {
            ...
            positions.Add(new Position(m_position.X - 1, m_position.Y - 1));
            positions.Add(new Position(m_position.X - 1, m_position.Y + 1));
            positions.Add(new Position(m_position.X + 1, m_position.Y - 1));
            positions.Add(new Position(m_position.X + 1, m_position.Y + 1));
